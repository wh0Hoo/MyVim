...
- 빌드·증분·검색이 쓰는 DB 는 **파일이 속한 프로젝트**로 정해진다:
  파일 디렉토리에 `GTAGS` 가 있으면 그것, 없으면 **프로젝트 루트**(`.projectroot` →
  `.git` 디렉토리 → `.hg` → `.svn` 순으로 상향 탐색)의 `GTAGS`
- `.git` 은 디렉토리일 때만 인정하므로 submodule 안을 편집해도 슈퍼프로젝트 루트를 잡는다
- 검색은 **주 DB + 등록한 DB 전체**를 한 번에 커버(전부 검색). 다른 프로젝트를 split 으로
  열어 그 심볼까지 검색하려면 그 버퍼에서 `:GtagsAdd` 로 검색 집합에 추가한다

| 명령 | 동작 |
|------|------|
| `:GtagsAdd` | 현재 버퍼의 프로젝트 `GTAGS` 를 검색 집합에 추가 (수동 등록) |
| `:Cs db add <경로>[::<루트>]` | 검색 집합에 DB 추가 (`::<루트>` 생략 가능) |
| `:Cs db rm <경로>[::<루트>]` | 검색 집합에서 DB 제거 |
| `:Cs db show` | 등록된 DB 목록 표시 (인자 없음) |
...
| 항목 | 값 |
|------|----|
| 탭 크기 | 기본 tab 8칸 (실제 탭 문자); 프로젝트에 `.editorconfig` 가 있으면 그에 따름 |
| 줄 번호 | 절대 번호 표시 |
| 마우스 | 기본 비활성 (`<Space>tm`으로 켜기) |
| 클립보드 | 시스템 클립보드 미연동 (기본) |
| 창 분할 방향 | 오른쪽 / 아래 |
| 스크롤 여백 | 8줄 (`scrolloff=8`) |
| 마지막 편집 위치 | 파일 재열기 시 자동 복원 (커밋 메시지 등은 제외) |
| 구문 강조 | Treesitter (C/C++, Lua, Python, JS/TS, Bash, Vim, help) |
