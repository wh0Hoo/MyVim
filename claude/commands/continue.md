...
Options:
  ...
  last          Show the 3 most recent sessions and let you pick which to restore
  help          Show this help
...
Run list-sessions with `--limit 4` (to account for the current session). Exclude the session where `isCurrent: true`. Display the remaining sessions (up to 3) as a numbered list using the same format as Step 1, but without pagination. Wait for user selection, then proceed to Step 2 with the selected session(s).

If fewer than 1 non-current session exists, print "복원할 이전 세션이 없습니다." and stop.
...
node ${WH0HOO_CC_ROOT}/scripts/list-sessions.js "${TRANSCRIPTS_DIR}" --limit 11 --offset 0
```
...
node ${WH0HOO_CC_ROOT}/scripts/preprocess.js "${TRANSCRIPT_PATH}"
```

The cache file is at:
```bash
PROJECT_HASH=$(echo "${PWD}" | sed 's/[^a-zA-Z0-9]/-/g')
CACHE_FILE="${HOME}/.claude/wh0Hoo/${PROJECT_HASH}/${SESSION_ID}/compact.txt"
```
...
- Preprocessed caches: ~/.claude/wh0Hoo/{PROJECT_HASH}/{SESSION_ID}/compact.txt
